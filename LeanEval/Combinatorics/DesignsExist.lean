...
@[eval_problem] theorem keevash_resolvable (q r lam : ℕ+) (hrq : r ≤ q) :
...
@[eval_problem] theorem keevash_large_set (q r : ℕ+) (hrq : r ≤ q) :
...
@[eval_problem] theorem keevash_complete_resolution (q : ℕ+) :
    ∃ N : ℕ, ∀ n > N, (∀ i ≤ q, (i : ℕ) ∣ (n - q)) →
