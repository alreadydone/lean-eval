...
subspaces of ℚⁿ. The specialization n = 1 gives the Thue–Siegel–Roth theorem
...
The slides https://pub.math.leidenuniv.nl/~evertsejh/14-bonn.pdf presents
...
In our statement, we use an arbitrary Fintype σ instead of `Fin n`, and use the
fact that every proper subspace is contained in a codimension 1 subspace, which is
described by the vanishing of a linear form with coefficients in ℤ. -/
@[eval_problem] theorem schmidt_subspace (σ : Type*) [Fintype σ] (L : σ → σ → ℂ)
